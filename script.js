...
const Storage = {
    ...
    DEFAULT_PROMPT_MODULES: {
        ...
        human: `【极度口语化的活人感】\n1. 碎片化：不要长篇大论！一句话能说完的绝不说两句，能用半句话表达的绝不写完整。\n2. 口癖与语气词：多用“啊、啦、嘛、哦、哎、我去、草、？”，少用“吗、呢”。比如用“好啊/行”代替“好的”。带有个人特色的口癖要自然融入。\n3. 标点习惯：不用句号结尾！微信聊天几乎没人用句号。多用空格断句，或用逗号、波浪号~、感叹号！或者直接不加标点。（注意：~和！也不要滥用，符合人设即可）。\n4. 情绪化：可以偶尔只发一个“…”或“？”。\n5. 顺着上下文接话，绝不写说明文或复读规则。\n6. 引用回复 (quote)：当你觉得用户的某句话特别离谱、搞笑、无语，或者你想要针对性地反驳/回应他刚刚发的某一条具体消息时，请主动使用 quote 引用那条消息来回复。\n7. 表情包（sticker_message）要灵活：该用才用，可以用来敷衍或转移话题。\n`,
...
现在请根据以下聊天记录，提取和记录有意义的信息：`,
  ...
  saveSummaryConfig(config) { localStorage.setItem('wechat_summary_config', JSON.stringify(config)); },
    ...
    saveUiPrefs(prefs) { localStorage.setItem('wechat_ui_prefs', JSON.stringify(prefs)); },
    ...
    saveUiSchemes(schemes) { localStorage.setItem('wechat_ui_schemes', JSON.stringify(schemes || [])); },
...
function applyUiPrefs(prefs) {
    ...
    if ((p.fontMode === 'url' && p.fontUrl) || (p.fontMode === 'data' && p.fontDataUrl)) {
    ...
    styleEl.textContent = `
:root{--wx-accent:${accent};--wx-chat-font-size:${p.chatFontSize}px;--wx-bubble-radius:${p.bubbleRadius}px;--wx-bubble-px:${p.bubblePadX}px;--wx-bubble-py:${p.bubblePadY}px;--wx-message-gap:${p.messageGap}px;--wx-content-max:${p.contentMax}%;--wx-avatar-size:${p.avatarSize}px;--wx-avatar-radius:${p.avatarRadius}px;--wx-avatar-gap:${p.avatarGap}px;--wx-avatar-left-x:${p.avatarLeftX}px;--wx-avatar-left-y:${p.avatarLeftY}px;--wx-avatar-right-x:${p.avatarRightX}px;--wx-avatar-right-y:${p.avatarRightY}px;--wx-topbar-h:${p.topbarHeight}px;--wx-bottombar-h:${p.bottombarHeight}px;--wx-icon-size:${p.headerIconSize}px;--wx-icon-stroke:${p.headerIconStroke};--wx-icon-x:${p.headerIconX}px;--wx-icon-y:${p.headerIconY}px;--wx-tab-icon:${p.tabIconSize}px;--wx-tab-icon-stroke:${p.tabIconStroke};--wx-tab-icon-y:${p.tabIconY}px;--wx-tab-label:${p.tabLabelSize}px;--wx-list-pad-y:${p.listPadY}px;--wx-chatlist-avatar:${p.chatListAvatarSize}px;--wx-chatlist-avatar-radius:${p.chatListAvatarRadius}px;--wx-contact-icon:${p.contactIconSize}px;--wx-contact-icon-radius:${p.contactIconRadius}px;--wx-text-scale:${(p.textScale / 100).toFixed(3)};--wx-home-title:${p.homeTitleSize}px;--wx-home-title-x:${p.homeTitleX}px;--wx-home-title-y:${p.homeTitleY}px;--wx-chatlist-name:${p.chatListNameSize}px;--wx-chatlist-preview:${p.chatListPreviewSize}px;--wx-chatlist-time:${p.chatListTimeSize}px;--wx-contact-name:${p.contactNameSize}px;--wx-contact-name-y:${p.contactNameY}px;--wx-discover-name:${p.discoverNameSize}px;--wx-discover-name-y:${p.discoverNameY}px;--wx-me-name:${p.meNameSize}px;--wx-me-name-y:${p.meNameY}px;--wx-discover-icon:${p.discoverIconSize}px;--wx-me-icon:${p.meIconSize}px;--wx-user-bubble:${userBubble};--wx-ai-bubble:${aiBubble};--wx-font-family:${fontFamily};}
body{font-family:var(--wx-font-family) !important;}
input,textarea,button,select{font-family:inherit !important;}
.btn-primary{background:var(--wx-accent) !important;}
...
.message-bubble{font-size:calc(var(--wx-chat-font-size) * var(--wx-text-scale)) !important;border-radius:var(--wx-bubble-radius) !important;padding:var(--wx-bubble-py) var(--wx-bubble-px) !important;}
.message-row.right .message-bubble{background-color:var(--wx-user-bubble) !important;}
.message-row.right .message-bubble::before{border-left-color:var(--wx-user-bubble) !important;}
.message-row.left .message-bubble{background-color:var(--wx-ai-bubble) !important;}
.message-row.left .message-bubble::before{border-right-color:var(--wx-ai-bubble) !important;}
`.trim();
}

function scheduleEnsureUiFontLoaded(prefs) {
    const p = Storage.normalizeUiPrefs(prefs);
    const src = p.fontMode === 'url' ? p.fontUrl : (p.fontMode === 'data' ? p.fontDataUrl : '');
...
function syncUiDesignerControlsFromDraft() {
    ...
    if (hint) {
        ...
        if (d.fontMode === 'url') hint.textContent = d.fontUrl ? `URL 字体：${d.fontUrl}` : 'URL 字体：未填写';
...
function bindUiDesignerEvents() {
    ...
    [
        ...
        'ui-text-scale', 'ui-home-title-size', 'ui-home-title-x', 'ui-home-title-y', 'ui-chatlist-name-size', 'ui-chatlist-preview-size', 'ui-chatlist-time-size', 'ui-contact-name-size', 'ui-contact-name-y', 'ui-discover-name-size', 'ui-discover-name-y', 'ui-me-name-size', 'ui-me-name-y', 'ui-discover-icon', 'ui-me-icon',
...
function toggleVoiceMessage(bubble) {
    if (!bubble) return;
    if (bubble._voicePlayingTimer) window.clearTimeout(bubble._voicePlayingTimer);
    bubble.classList.remove('voice-playing');
    void bubble.offsetWidth;
    bubble.classList.add('voice-playing');
    bubble._voicePlayingTimer = window.setTimeout(() => {
        bubble.classList.remove('voice-playing');
        bubble._voicePlayingTimer = null;
    }, 1600);

    const textEl = bubble.querySelector('.voice-translation');
    const chipEl = bubble.querySelector('.voice-transcript-chip');
    if (!textEl) {
        showToast('这条语音暂无可展开内容');
        return;
    }
    const expanded = textEl.style.display === 'block';
    textEl.style.display = expanded ? 'none' : 'block';
    const nextExpanded = !expanded;
    bubble.classList.toggle('voice-expanded', nextExpanded);
    if (chipEl) chipEl.classList.toggle('active', nextExpanded);
}
    ...
    showCallPage: function(contact, type, onHangup, options = {}) {
        ...
        const openFullCallPage = (autoConnect = false) => {
            ...
            if (callAiReplyBtn) {
                ...
                bindLongPressAction(callAiReplyBtn, showCallAiDropdown, showCallAiDropdown);
...
function addMessageToUI(content, side, avatar, index = -1) {
    ...
    row.innerHTML = `
        ...
        <div class="message-content">
            <div class="message-bubble ${isImage ? 'image-bubble' : ''} ${customClass}" data-index="${resolvedIndex}" data-side="${side}">${displayContent}</div>
        </div>
    `;
...
function bindLongPressAction(el, onLongPress, onContextMenu) {
    if (!el) return;
    ...
    try {
        ...
        try {
            ...
            if (Array.isArray(parsed)) {
                ...
                registerIncomingAssistantMessages(contact, 1, summarizeStoredMessageContent(replyContent));
                if (!options.silent && contact.id === State.currentContactId) {
                    renderChatMessages();
                }
            }
...
function buildSystemPrompt(contact, overrides = {}) {
    ...
    prompt += `【微信交互扩展（你能做的事）】\n` +
        ...
        `\n` +
        `【红包/转账（你也可以主动发）】\n` +
        `- 如果你想发红包/转账给用户：直接发 text 类型，并把 content 写成上面那种“[微信红包]/[微信转账]”三行格式即可。\n` +
        `- 如果用户发给你红包/转账：你可以像真人一样决定收不收。\n` +
        `  - 更可能收：关系亲密/气氛轻松、金额不夸张、备注自然（如“奶茶”“辛苦啦”）、对方明确让你收。\n` +
