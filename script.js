...
const Storage = {
    ...
    DEFAULT_PROMPT_SCENARIO_TEMPLATES: {
        proactive: `（系统）这是一次“主动发消息”场景，不是在回复用户刚发来的内容。
对方已经 {interval_min} 分钟没有理你了，你心里有一点想念对方。
系统为你提供的当前实时时间是：{date_time} ({timezone})。当前时间段：{period_label}。
请先结合当前时间段和前面的聊天记录，再自然地发 1 条简短消息找对方，或者顺手分享一下你自己的日常。
...
你看到用户「{user}」发布了一条动态。请像真实朋友圈一样，决定是否点赞，以及是否发表评论。
要求：
- 输出严格 JSON（不要带多余文字，不要 Markdown）。
...
请生成这条动态下的一批 NPC 点赞、评论和少量评论互动。`,
        callSystem: `你现在扮演「{char}」，正在与用户进行{call_kind}。
...
- 完全代入角色，不限制回复长短，根据当前场景和情绪自然表达。`,
        callAutoReply: `（系统）以下是用户刚刚在通话里说的话，请你统一自然接话并回复：
{pending_user_text}`,
...
function getScenarioPromptVars(contact, extra = {}) {
    ...
    const vars = {
        char: roleName,
        user: userProfile?.nickname || State.user?.nickname || '用户',
...
function buildSystemPrompt(contact, overrides = {}) {
    ...
    if (summaryMemory) {
        const summaryMemoryAnchor = `以下是你们以往聊天里整理出的记忆片段。阅读时请固定理解：这里的“我”始终指${roleName}，这里的“ta”始终指${userProfile.nickname || '用户'}。`;
        prompt += `【已归档记忆】\n${summaryMemoryAnchor}\n${summaryMemory}\n\n`;
    }
...
function buildMomentPostPromptPair(contact, overrides = {}) {
    ...
    return {
        systemPrompt: renderPromptTemplate(templates.momentsPostSystem || Storage.DEFAULT_PROMPT_SCENARIO_TEMPLATES.momentsPostSystem, vars),
...
function buildMomentInteractionPromptPair(contact, moment) {
    ...
    return {
        systemPrompt: renderPromptTemplate(templates.momentsInteractionSystem || Storage.DEFAULT_PROMPT_SCENARIO_TEMPLATES.momentsInteractionSystem, vars),
...
function buildMomentReplyPromptPair(authorContact, moment, commentsToReply) {
    ...
    return {
        systemPrompt: renderPromptTemplate(templates.momentsReplySystem || Storage.DEFAULT_PROMPT_SCENARIO_TEMPLATES.momentsReplySystem, vars),
...
function buildMomentSocialPromptPair(contact, moment, overrides = {}) {
    ...
    return {
        systemPrompt: renderPromptTemplate(templates.momentsSocialSystem || Storage.DEFAULT_PROMPT_SCENARIO_TEMPLATES.momentsSocialSystem, vars),
...
function buildCallSystemPrompt(contact, type, summaryMemoryText = '') {
    ...
    return renderPromptTemplate(templates.callSystem || Storage.DEFAULT_PROMPT_SCENARIO_TEMPLATES.callSystem, vars);
}
