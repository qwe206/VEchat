...
{pending_user_text}`,
        summary: `你是一个智能记忆助手，负责从聊天记录中提取和记录有意义的信息。你的任务不是简单地总结对话流水账，而是像记事本一样记录重要的、有价值的信息。
    ...
    getPromptModules() {
        ...
        try {
            ...
            if (protocolPatched !== protocol) {
                ...
                try {
                    localStorage.setItem('wechat_prompt_modules', JSON.stringify({ ...(parsed || {}), protocol: merged.protocol }));
                } catch (e) {}
            }
            return merged;
        } catch (e) {
...
function buildPromptPreviewText(previewContact, mode) {
    ...
    if (previewMode === 'summary') {
        ...
        return buildSummaryPromptText(basePrompt, '最近你们刚和好，用户这周在赶作业。', '[user]: 我今晚还得写报告\n[assistant]: 那你先忙完再说');
    }
    return buildSystemPrompt(previewContact);
}
...
function bindChatEvents() {
    ...
    bindLongPressAction(batchReplyBtn, () => {
        ...
        if (plusPanel) plusPanel.classList.remove('show');
        showAiReplyDropdown(contact, { allowMoneyActions: true }, batchReplyBtn);
    }, { thresholdMs: AI_QUICK_MENU_LONG_PRESS_MS });
...
async function triggerAutoSummary(contact) {
    ...
    const latestSummaryContent = previousSummaries.length > 0
        ? String(previousSummaries[previousSummaries.length - 1]?.content || '').trim()
        : '';
    const summaryPrompt = buildSummaryPromptText(basePrompt, latestSummaryContent, chatTextForSummary);
    rememberPromptTrace(contact.id, 'summary', summaryPrompt);
...
function rememberPromptTrace(contactId, mode, content, extra = {}) {
    ...
    const existing = State.lastPromptTraces[id][mode];
    const history = Array.isArray(existing)
        ? existing
        : (typeof existing === 'string' && existing.trim() ? [{ content: existing.trim(), at: Date.now(), label: meta.label, category: meta.category, mode }] : []);
    history.push({
        mode,
    ...
    State.lastPromptTraces[id][mode] = history.slice(-20);
}
...
function getPromptTraceEntries(contactId, category) {
    ...
    Object.entries(traces).forEach(([mode, value]) => {
        const list = Array.isArray(value) ? value : (typeof value === 'string' && value.trim()
        ...
        list.forEach((item, idx) => {
            ...
            entries.push({
                id: `${mode}_${item?.at || Date.now()}_${idx}`,
                mode: item?.mode || mode,
                category: meta.category,
