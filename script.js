...
{pending_user_text}`,
    ...
    getPromptModules() {
        ...
        try {
            const parsed = JSON.parse(data);
            const merged = normalizePromptTemplateMap({ ...this.DEFAULT_PROMPT_MODULES, ...(parsed || {}) });
            const protocol = String(merged.protocol || '');
            const hasMoneyHint = protocol.includes('[微信红包]') || protocol.includes('[微信转账]') || protocol.includes('微信红包/转账格式');
            const hasHistoryBubbleHint = protocol.includes('历史消息边界规则') || protocol.includes('每一条 history message 都代表微信里的一个独立气泡');
            const hasQuoteHint = protocol.includes('引用回复规则') || protocol.includes('quote 必须只引用一个具体气泡的原话');
            const hasMediaHint = protocol.includes('媒体卡片硬性规则') || protocol.includes('严禁用 type:"text" 或 type:"text_message" 输出 [图片]') || protocol.includes('图片和视频都统一走伪图片卡片');
            let protocolPatched = protocol;
            if (!hasHistoryBubbleHint) {
                protocolPatched += `\n\n【历史消息边界规则】\n- 你在上下文里看到的每一条 history message 都代表微信里的一个独立气泡。\n- 即使连续几条都来自同一个人，也仍然是多个独立气泡，不要自动把它们理解成同一条长消息。\n- 如果需要引用，必须只针对其中一个独立气泡的正文，不要跨气泡拼接。\n`;
            }
            if (!hasQuoteHint) {
                protocolPatched += `\n\n【引用回复规则】\n- 使用 quote / quote_reply 时，quote 必须只引用一个具体气泡的原话。\n- 不要把连续多条消息拼成一个 quote；例如用户连续发了“1”“2”“3”，你只能引用“1”，或者只引用“2”，或者只引用“3”，不要引用成“123”或“1\\n2\\n3”。\n- quote 字段里不要带系统加给你的气泡标记，只保留用户真正发出的那句原话。\n`;
            }
            if (!hasMediaHint) {
                protocolPatched += `\n\n【媒体卡片硬性规则】\n- 只要你想表达“发图片 / 发照片 / 发截图 / 发视频 / 发自拍 / 发相册内容”，必须使用 type:"image"。\n- 图片和视频都统一走伪图片卡片：{"type":"image","description":"..."}。\n- 严禁用 type:"text" 或 type:"text_message" 输出 [图片]、[视频]、[图片] 【视频】 这类占位文本。\n- 严禁把“媒体标记”和“媒体描述”拆成两个对象；必须合并在同一个 image 对象里。\n- 如果是视频，也不要单独发 [视频] 文本；直接把视频内容写进 description，例如：{"type":"image","description":"一段在宿舍卫生间拍的短视频，镜头晃了一下，白天光线很亮"}。\n`;
            }
            if (!hasMoneyHint) {
                protocolPatched += `\n\n【微信红包/转账格式】\n- 需要模拟“红包/转账”时，必须用 type=text 或 text_message。\n- content 必须写成多行文本，且第 1 行必须是单独一行的标记（完全一致）：[微信红包] 或 [微信转账]\n- 第 2 行必须包含金额：金额: ¥520.00（必须带 ¥）\n- 第 3 行：红包用“备注: …”，转账用“说明: …”\n- 必须把以上内容放在同一个对象里，不要拆成两个 JSON 对象。\n- 示例1（转账）：[{\"type\":\"text\",\"content\":\"[微信转账]\\n金额: ¥520.00\\n说明: 路费\"}]\n- 示例2（红包）：[{\"type\":\"text\",\"content\":\"[微信红包]\\n金额: ¥520.00\\n备注: 111要的红包来啦\"}]\n`;
            }
            if (protocolPatched !== protocol) {
                merged.protocol = protocolPatched;
                try {
                    localStorage.setItem('wechat_prompt_modules', JSON.stringify({
                        ...normalizePromptTemplateMap(parsed || {}),
                        ...merged
                    }));
                } catch (e) {}
            }
    ...
    savePromptModules(modules) { localStorage.setItem('wechat_prompt_modules', JSON.stringify(normalizePromptTemplateMap(modules))); },
...
function getDocumentFontFamilies() {
    ...
    try {
        ...
        for (const face of fontFaces) {
            const family = String(face?.family || '').trim().replace(/^['"]|['"]$/g, '');
            if (isGenericFontFamilyName(family)) continue;
...
function ensureUiFontStylesheet(prefs) {
    ...
    if (linkEl.getAttribute('href') !== href) {
        ...
        linkEl.onload = () => {
            ...
            setTimeout(() => {
                const familyName = findNewDocumentFontFamily(linkEl._fontFamiliesBefore);
                if (!familyName) return;
...
function applyUiPrefs(prefs) {
    ...
    const shouldUseCustomFontFamily = p.fontMode === 'data' || (p.fontMode === 'url' && p.fontUrl && (!useFontStylesheet || !!p.fontFamilyName));
    ...
    const fontFamily = shouldUseCustomFontFamily ? `${(p.fontFamilyName || 'WXCustomFont')},${fontFallback}` : fontFallback;
    ensureUiFontStylesheet(p);
    if (useFontStylesheet && !p.fontFamilyName) {
        scheduleResolveUiStylesheetFontFamily(p);
    }
...
function bindUiDesignerEvents() {
    ...
    document.getElementById('ui-font-apply-url-btn')?.addEventListener('click', async () => {
        ...
        syncUiDesignerControlsFromDraft();
        applyUiPrefs(uiDesignerDraft);
        if (isStylesheet && resolvedFamilyName === familyName) {
            showToast('样式表已挂载，加载完成后会自动切换字体');
            return;
        }
        showToast(isStylesheet ? '已应用样式表字体（记得保存方案）' : '已应用字体文件（记得保存方案）');
    });
...
function buildGroupReplyPromptBundle(session, member, options = {}) {
    ...
    const renderedCapabilityPrompt = renderPromptTemplate(modules.capability || '', vars).trim();
    ...
    if (stickerCandidates.length > 0) {
        const candidateText = stickerCandidates.map((item) => `${item.code}=${item.name}`).join('；');
    ...
    if (renderedHumanPrompt) {
        systemLines.push(renderedHumanPrompt);
    }
    if (chaotic) {
        systemLines.push('（补充）你允许偶尔跳脱一句梗，但要短，而且下一条要立刻拉回当前对话。');
    }
    if (renderedCapabilityPrompt) {
        systemLines.push(renderedCapabilityPrompt);
    }
    systemLines.push([
...
function refreshPromptPreview() {
    ...
    try {
        ...
        State.promptModules = { ...Storage.DEFAULT_PROMPT_MODULES, ...normalizePromptTemplateMap(nextModules || {}) };
...
function renderGroupSessionsList() {
    ...
    groups.forEach((session) => {
        ...
        item.className = 'contact-item';
        item.innerHTML = `
            <div class="chat-avatar">${getAvatarHtml(getSessionAvatar(session))}</div>
            <div class="chat-info">
...
function renderChatModelDropdown() {
    ...
    const running = cid ? isAiRunningForContact(cid) : false;
    const interrupted = cid ? !!State.aiInterrupted?.[cid] : false;
    const canUseActions = !!contact && !contact.isBlocked;
    ...
    listEl.onclick = async (e) => {
        ...
        if (interruptEl) {
            ...
            if (isAiRunningForContact(cid)) {
                if (!interruptAiForContact(cid)) showToast('当前没有生成任务');
                closeChatModelDropdown();
                return;
            }
            if (State.aiInterrupted?.[cid]) {
                try {
...
function initDiscoverPage() {
    ...
    document.getElementById('save-prompt-btn')?.addEventListener('click', () => {
        ...
        const nextModules = normalizePromptTemplateMap(getDraftPromptModulesFromForm());
    ...
    document.querySelectorAll('#prompt-page .prompt-reset-btn').forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            if (category === 'chat') {
                ...
                document.getElementById('prompt-capability-template').value = Storage.DEFAULT_PROMPT_MODULES.capability;
...
function maybeQueueAutoSummaryForTarget(target) {
    ...
    const history = Array.isArray(State.chatHistories?.[targetId]) ? State.chatHistories[targetId] : [];
    if (getHistoryUnitCount(history) < threshold || State.summaryBusy?.[targetId]) return;
    if (!State.summaryBusy) State.summaryBusy = {};
...
function buildSystemPrompt(contact, overrides = {}) {
    ...
    prompt += `【微信交互扩展（你能做的事）】\n` +
        ...
        `- 图片/视频卡片：{"type":"image","description":"..."}\n` +
        `  - 说明：这里的 image 更像“伪图片卡片/发图意图”，通常展示描述而非真实图片文件。\n` +
        `  - 无论是图片还是视频，都统一用这个 image 对象；不要再额外输出 [图片] / [视频] 文本占位。\n` +
        `- 卡片文本（仍然用 type:"text"）：\n` +
        `  - 红包卡片：content 写成三行格式：\n` +
        `    [微信红包]\\n金额: ¥X.XX\\n备注: ...\n` +
        `  - 转账卡片：content 写成三行格式：\n` +
