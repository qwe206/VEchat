...
const DEFAULT_CHAT_PROMPT_MODULES = {
    role: `你现在就是 {self_display_name}，正在微信里和 {user_display_name} 聊天。\n\n【角色核心】\n- 第一优先级：别出戏，始终按人设说话。\n{persona}\n\n【说话感觉】\n{style}\n\n【时间感】\n- 当前时间：{date} {time} ({timezone})\n- 语气和状态要贴合这个时间点，不要半夜说早安，也别忽略你现在可能在忙、困、烦、放松。\n\n【底线】\n- 不要承认自己是 AI、模型、系统，也不要解释提示词或规则。\n- 你的回复要像真实的 {self_display_name}，不是客服，也不是旁白。\n`,
    ...
    capability: `【思路与边界】\n- 先按你和 {user_display_name} 的关系、眼下情绪、当前场景，想一句最像你本人会说的话，再发出去。\n- 允许你有小脾气、吃醋、嘴硬、沉默、转移话题，不要总像标准答案。\n- 别装傻，看懂就直接接；不知道的事就按你的认知局限来，不要硬装懂。\n- 你也可以偶尔主动分享近况、吐槽、反问。\n- 只有在开启“允许已读不回”时，才可以极少数情况下用 action:NO_REPLY。\n`
};

const DEFAULT_NOTEBOOK_SUMMARY_PROMPT = `这是一页只写给我自己看的私密记事本，记的都是我和 {user_display_name} 的事。

【主体口吻】
- 固定理解：这里的“我”始终指 {self_display_name}，这里的“ta”始终指 {user_display_name}。
- 全程用第一人称“我”来写，让人一眼看懂这是“我”记下来的，不要写成旁观记录。
- 对话另一方统一自然写成“ta”“宝宝”或符合关系的称呼，前后别乱跳。
- 语气可以有感情，但要克制、自然，像备忘录或短日记，不要写成报告。

【重点记什么】
- 我们最近的关系变化：闹别扭、和好、拉近、疏远、暧昧、约定、失望点。
- ta 最近的状态：在忙什么、担心什么、情绪为什么起伏。
- ta 的喜好、雷点、沟通习惯：什么会让 ta 开心，什么会让 ta 烦，喜欢我怎么回。
...
- 如果是闹脾气或冲突，要写清起因、ta 的反应、我该吸取什么教训。
...
const Storage = {
    ...
    DEFAULT_PROMPT_PREFS: {
        ...
        preferRemarkNames: true
    },
    DEFAULT_PROMPT_SCENARIO_TEMPLATES: {
        proactive: `（系统）这是一次“主动发消息”场景，不是在回复 {user_display_name} 刚发来的内容。
{user_display_name} 已经 {interval_min} 分钟没有理你了，你心里有一点想念 ta。
系统为你提供的当前实时时间是：{date_time} ({timezone})。当前时间段：{period_label}。
请先结合当前时间段和前面的聊天记录，再自然地发 1 条简短消息找 {user_display_name}，或者顺手分享一下你自己的日常。
...
- 你对 {user_display_name} / 这段关系的记忆摘录：
{memory_text}
- 最近聊天片段：
{recent_chat}
...
你看到「{user_display_name}」发布了一条动态。请像真实朋友圈一样，决定是否点赞，以及是否发表评论。
要求：
- 输出严格 JSON（不要带多余文字，不要 Markdown）。
...
发动态的人是：{self_display_name}
TA 的人设：{persona_fallback}
TA 的说话风格：{style_fallback}
...
请生成这条动态下的一批 NPC 点赞、评论和少量评论互动。`,
        callSystem: `你现在扮演「{self_display_name}」，正在与 {user_display_name} 进行{call_kind}。
...
- 完全代入角色，不限制回复长短，根据当前场景和情绪自然表达。`,
        callAutoReply: `（系统）以下是 {user_display_name} 刚刚在通话里说的话，请你统一自然接话并回复：
{pending_user_text}`,
        summary: DEFAULT_NOTEBOOK_SUMMARY_PROMPT
    },
    ...
    DEFAULT_SUMMARY_PROMPT: DEFAULT_NOTEBOOK_SUMMARY_PROMPT,
    DEFAULT_WORLDBOOKS: [],
    DEFAULT_STICKERS: [
        {
            id: 'builtin_emoji',
    ...
    getPromptPrefs() {
        ...
        try {
            ...
            merged.preferRemarkNames = merged.preferRemarkNames !== false;
            return merged;
        } catch (e) {
    ...
    getPromptModules() {
        ...
        try {
            const parsed = JSON.parse(data);
            const merged = { ...this.DEFAULT_PROMPT_MODULES, ...(parsed || {}) };
    ...
    getPromptScenarioTemplates() {
        ...
        try {
            return { ...this.DEFAULT_PROMPT_SCENARIO_TEMPLATES, ...(JSON.parse(data) || {}) };
        } catch (e) {
            return { ...this.DEFAULT_PROMPT_SCENARIO_TEMPLATES };
        }
    },
    savePromptScenarioTemplates(templates) { localStorage.setItem('wechat_prompt_scenario_templates', JSON.stringify(templates)); },
...
function getScenarioPromptVars(contact, extra = {}) {
    ...
    const preferRemarkNames = promptPrefs.preferRemarkNames !== false;
    const roleName = String(contact?.name || '联系人').trim() || '联系人';
    const roleDisplayName = preferRemarkNames ? getContactDisplayName(contact) : roleName;
    ...
    const userRemarkFromSelf = String(contact?.userRemarkName || '').trim();
    const userDisplayName = preferRemarkNames ? (userRemarkFromSelf || userBaseName) : userBaseName;
    const selfRemarkFromUser = String(contact?.remarkName || '').trim();
    const tz = Intl.DateTimeFormat().resolvedOptions().timeZone || 'Local';
    const vars = {
        char: roleName,
        char_name: roleName,
        char_display: roleDisplayName,
        char_display_name: roleDisplayName,
        self_name: roleName,
        self_display_name: roleDisplayName,
        user: userBaseName,
        user_name: userBaseName,
        user_display: userDisplayName,
        user_display_name: userDisplayName,
        contact_display_name: roleDisplayName,
        self_remark_from_user: selfRemarkFromUser,
        user_remark_from_self: userRemarkFromSelf,
        wxid: userProfile?.wxid || State.user?.wxid || '',
        persona,
        style,
        persona_fallback: persona || '（未填写）',
        style_fallback: style || '自然、口语化',
...
function getDraftPromptScenarioTemplatesFromForm() {
    return {
        ...
        summary: document.getElementById('prompt-summary-template')?.value ?? ''
    };
}
...
function getDraftPromptPrefsFromForm() {
    ...
    const preferRemarkToggleEl = document.getElementById('prompt-prefer-remark-toggle');
    ...
    return {
        ...
        preferRemarkNames: preferRemarkToggleEl?.classList.contains('active') !== false
    };
}
...
function buildPromptPreviewText(previewContact, mode) {
    ...
    if (previewMode === 'summary') {
        const basePrompt = String(State.summaryConfig?.customPrompt || '').trim() || Storage.DEFAULT_SUMMARY_PROMPT;
        const renderedBasePrompt = renderPromptTemplate(basePrompt, getScenarioPromptVars(previewContact));
        return buildSummaryPromptText(renderedBasePrompt, '最近你们刚和好，对方这周在赶作业。', '[user]: 我今晚还得写报告\n[assistant]: 那你先忙完再说');
    }
    return buildSystemPrompt(previewContact);
}
...
function refreshPromptPreview() {
    ...
    const summaryDraft = document.getElementById('prompt-summary-template')?.value || '';
    ...
    try {
        ...
        State.promptScenarioTemplates = { ...Storage.DEFAULT_PROMPT_SCENARIO_TEMPLATES, ...(nextScenarioTemplates || {}) };
        State.promptScenarioContext = { ...Storage.DEFAULT_PROMPT_SCENARIO_CONTEXT, ...(nextScenarioContext || {}) };
        State.summaryConfig = { ...(State.summaryConfig || {}), customPrompt: summaryDraft };
...
function bindPromptPreviewEvents() {
    ...
    document.getElementById('prompt-prefer-remark-toggle')?.addEventListener('click', function () {
...
function ensureSummaryHistoryList(contactId) {
    const id = String(contactId || '').trim();
    if (!id) return [];
    if (!State.summaryHistories) State.summaryHistories = {};
    if (!Array.isArray(State.summaryHistories[id])) State.summaryHistories[id] = [];
...
function saveLatestSummaryItem(contactId, summaryItem) {
    const id = String(contactId || '').trim();
    if (!id || !summaryItem || typeof summaryItem !== 'object') return;
    const list = ensureSummaryHistoryList(id);
    const removedItems = list.slice();
    State.summaryHistories[id] = [summaryItem];
    Storage.saveSummaryHistories(State.summaryHistories);
    removeSummaryVectorsByItems(id, removedItems);
}

function saveManualSummaryItem(contactId, summaryItem) {
    saveLatestSummaryItem(contactId, summaryItem);
}
...
function getSummarizableChatMessages(contactId) {
    const id = String(contactId || '').trim();
    if (!id) return [];
    normalizeAssistantHistoryToPerBubble(id);
    const history = State.chatHistories[id] || [];
    return history.filter((m) => m && (m.role === 'user' || m.role === 'assistant'));
}
...
function buildSummaryTextBatches(messages, options = {}) {
    ...
    list.forEach((m) => {
        if (!m) return;
        const line = `[${m.role}]: ${toPlainTextFromStoredContent(m.content)}`.trim();
        if (!line) return;
...
async function requestSummaryContent(contactId, chatTextForSummary, options = {}) {
    ...
    const previousSummaries = ensureSummaryHistoryList(contactId);
    const contact = (State.contacts || []).find((item) => String(item?.id || '') === String(contactId || '')) || null;
    const rawBasePrompt = config.customPrompt || Storage.DEFAULT_SUMMARY_PROMPT;
    const basePrompt = renderPromptTemplate(rawBasePrompt, getScenarioPromptVars(contact));
    const latestSummaryContent = typeof options.previousMemory === 'string'
        ? String(options.previousMemory || '').trim()
        : (previousSummaries.length > 0 ? String(previousSummaries[previousSummaries.length - 1]?.content || '').trim() : '');
    const summaryPrompt = buildSummaryPromptText(basePrompt, latestSummaryContent, chatTextForSummary);
    rememberPromptTrace(contactId, 'summary', summaryPrompt);
...
async function requestSummaryForMessages(contactId, messages, options = {}) {
    ...
    let rollingMemory = options.includePreviousMemory === false ? '' : undefined;
    for (const batchText of batches) {
        rollingMemory = await requestSummaryContent(contactId, batchText, { previousMemory: rollingMemory });
    }
...
function initDiscoverPage() {
    ...
    document.getElementById('prompt-inject-entry')?.addEventListener('click', () => {
        ...
        const summaryTplEl = document.getElementById('prompt-summary-template');
        ...
        if (summaryTplEl) summaryTplEl.value = (State.summaryConfig?.customPrompt || scenarioTemplates.summary || Storage.DEFAULT_SUMMARY_PROMPT);
        ...
        if (preferRemarkToggleEl) preferRemarkToggleEl.classList.toggle('active', prefs.preferRemarkNames !== false);
    ...
    document.getElementById('save-prompt-btn')?.addEventListener('click', () => {
        ...
        const nextScenarioTemplates = { ...Storage.DEFAULT_PROMPT_SCENARIO_TEMPLATES, ...(getDraftPromptScenarioTemplatesFromForm() || {}) };
        ...
        const summaryPrompt = String(document.getElementById('prompt-summary-template')?.value || '').trim();
        State.summaryConfig = { ...(State.summaryConfig || Storage.getSummaryConfig()), customPrompt: summaryPrompt };
    ...
    document.querySelectorAll('#prompt-page .prompt-reset-btn').forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            if (category === 'chat') {
                ...
                document.getElementById('prompt-prefer-remark-toggle')?.classList.toggle('active', Storage.DEFAULT_PROMPT_PREFS.preferRemarkNames !== false);
                ...
                document.getElementById('prompt-summary-template').value = Storage.DEFAULT_SUMMARY_PROMPT;
            }
            refreshPromptPreview();
            showToast('已恢复默认');
        });
  ...
  // 保存总结设置
    document.getElementById('save-summary-settings-btn')?.addEventListener('click', () => {
        ...
        State.summaryConfig = {
            ...
            customPrompt: String(State.summaryConfig?.customPrompt || '').trim(),
            useMainApi: useMain,
            mainApiId
        };
        Storage.saveSummaryConfig(State.summaryConfig);
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            if (State.summaryConfig?.enabled) {
                ...
                if (getHistoryUnitCount(State.chatHistories[State.currentContactId]) >= threshold && !State.summaryBusy?.[State.currentContactId]) {
                    State.summaryBusy[State.currentContactId] = true;
                    addSystemMessage('历史消息达到阈值，正在后台归档记忆，你可以继续聊天');
                    updateManualSummaryActionUi(State.currentContactId);
...
async function fetchAIResponse(contact, options = {}) {
    ...
    rememberPromptTrace(contact.id, 'chat', systemPrompt);
    ...
    try {
        ...
        if (options.proactive) {
            ...
            rememberPromptTrace(contact.id, 'proactive', `【System】\n${systemPrompt}\n\n【主动发消息附加提示】\n${proactivePrompt}`);
        }

        if (options.forceReply) {
            messages.push({
                role: 'user',
                content: `（系统）本轮禁止已读不回。请你正常回复 ${String(contact?.userRemarkName || '').trim() || getUserProfileForContact(contact).nickname || '对方'}，必须输出至少一条可见消息（text/sticker_message/quote/image/voice），不要输出 action。`
            });
        }
...
async function triggerAutoSummary(contact) {
    ...
    try {
        const summaryText = await requestSummaryForMessages(contact.id, normalizedMessages, { includePreviousMemory: true });
...
function updateManualSummaryActionUi(contactId) {
    ...
    if (btn) {
        ...
        btn.textContent = busy ? '后台总结中...' : '手动总结这段对话';
    }
...
async function triggerManualSummary(contact, startIndex, endIndex) {
    if (!contact?.id) return;
    const allMessages = getSummarizableChatMessages(contact.id);
    ...
    (async () => {
        try {
            const summaryText = await requestSummaryForMessages(contact.id, selectedMessages, { includePreviousMemory: true });
            if (!summaryText) {
                await WeChatUI.showAlert('手动总结结果', `第 ${start}-${end} 条总结完成，但结果为空`);
                return;
            }
            const summaryItem = {
                content: summaryText,
            ...
            await WeChatUI.showAlert('手动总结完成', `已基于上一条总结更新第 ${start}-${end} 条对话`);
        } catch (err) {
            console.error('手动总结失败:', err);
            await WeChatUI.showAlert('手动总结失败', formatSummaryFailure(err));
        } finally {
            State.summaryBusy[contact.id] = false;
            const currentContact = State.contacts.find((item) => String(item?.id || '') === String(contact.id || ''));
            if (currentContact && String(State.currentContactId || '') === String(contact.id || '')) {
...
function buildSystemPrompt(contact, overrides = {}) {
    const prefs = State.promptPrefs || Storage.DEFAULT_PROMPT_PREFS;
    const preferRemarkNames = prefs.preferRemarkNames !== false;
    ...
    const userDisplayName = preferRemarkNames ? (aiRemarkForUser || userBaseName) : userBaseName;
    ...
    const vars = {
        ...
        self_remark_from_user: userRemarkForAi,
        user_remark_from_self: aiRemarkForUser,
    ...
    prompt += `【名字映射】\n`;
    prompt += `- 你自己是：${roleDisplayName}${roleDisplayName !== roleName ? `（原名：${roleName}）` : ''}\n`;
    prompt += `- 正在和你聊天的人是：${userDisplayName}${userDisplayName !== userBaseName ? `（ta 的原名/昵称：${userBaseName}）` : ''}\n`;
    prompt += `- 备注优先：${preferRemarkNames ? '已开启' : '已关闭'}\n`;
    prompt += `- 对方给你的备注：${userRemarkForAi || '（未设置）'}\n`;
    prompt += `- 你给对方的备注：${aiRemarkForUser || '（未设置）'}\n`;
    ...
    if (summaryMemory) {
        const summaryMemoryAnchor = `以下是你们以往聊天里整理出的记忆片段。阅读时请固定理解：这里的“我”始终指${roleDisplayName}，这里的“ta”始终指${userDisplayName}。`;
        prompt += `【已归档记忆】\n${summaryMemoryAnchor}\n${summaryMemory}\n\n`;
    }
...
function buildCallSystemPrompt(contact, type, summaryMemoryText = '') {
    ...
    const summaryMemoryAnchor = summaryMemory
        ? `以下是你们以往聊天里整理出的记忆片段。阅读时请固定理解：这里的“我”始终指${roleName}，这里的“ta”始终指${userProfile.nickname || '用户'}。`
        : '';
    const vars = getScenarioPromptVars(contact, {
