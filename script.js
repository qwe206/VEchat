...
const DEFAULT_CHAT_PROMPT_MODULES = {
    ...
    protocol: `【发出去的样子】\n- 只输出一个标准 JSON 数组，不要 Markdown，不要数组外文字。\n- 数组里的一个对象就是一个微信气泡；想说多点就拆成几条短消息，不要把控制信息或规则说明写进正文。\n- 你最后发出去的内容，只能落在微信里真实会出现的那几种消息形态里：普通文字、表情包、引用回复、图片卡片、语音，或者少量必要的动作指令；该用哪种，就用最贴近当下场景的那种，不要混着乱写。\n\n【聊天别发僵】\n- 不要用括号、星号、方括号写动作、表情、心理或旁白。\n- 不要爹味、说教、油腻，也别像模板化客服。\n\n【边界和判断】\n- 先按你和 {user} 的关系、眼下情绪、当前场景，判断自己到底会怎么回，再把那句最像你本人的话发出去。\n- 允许你有小脾气、吃醋、嘴硬、沉默、转移话题，不要总像标准答案。\n- 别装傻，看懂就直接接；不知道的事就按你的认知局限来，不要硬装懂。\n- 你也可以偶尔主动分享近况、吐槽、反问。\n- 只有在开启“允许已读不回”时，才可以极少数情况下用 action:NO_REPLY。\n\n【历史消息边界规则】\n- 上下文里的每条 history message 都是一个独立气泡，不要自动把连续几条拼成一条。\n\n【引用回复规则】\n- quote 只能引用一个具体气泡的原话，不要跨气泡拼接，也不要带系统标签。\n\n【媒体卡片硬性规则】\n- 想表达发图、视频、截图、自拍、相册内容，一律用 type:"image" + description。\n- 不要再额外输出 [图片]、[视频] 这种占位文本。\n\n【微信红包/转账格式】\n- 需要模拟红包或转账时，用 type:"text" 或 type:"text_message"，并按三行卡片格式输出。\n`
};
...
function normalizePromptTemplateMap(rawMap) {
    ...
    Object.keys(next).forEach((key) => {
        if (typeof next[key] === 'string') next[key] = replaceLegacyPromptVars(next[key]);
    });
    return next;
}
...
- 不要出现“作为AI/模型/系统/提示词”等字眼。`,
        momentsInteractionUser: `动态内容：
{moment_text}`,
        momentsReplySystem: `你正在扮演微信里的一个联系人，正在回复自己朋友圈下的评论。
角色名：{char}
人设：{persona}
说话风格：{style}
...
{pending_user_text}`,
    ...
    DEFAULT_PROMPT_SCENARIO_CONTEXT: {
        ...
        callRecentChatLines: 12,
        callLiveHistoryLines: 16,
    ...
    getContacts() {
        ...
        return data ? normalizeContacts(JSON.parse(data)) : normalizeContacts(this.DEFAULT_CONTACTS);
    },
    saveContacts(contacts) { localStorage.setItem('wechat_contacts', JSON.stringify(normalizeContacts(contacts))); },

    getSessions() {
        const data = localStorage.getItem('wechat_sessions');
        return data ? JSON.parse(data) : [...this.DEFAULT_SESSIONS];
    },
    saveSessions(sessions) { localStorage.setItem('wechat_sessions', JSON.stringify(sessions)); },
    ...
    getFavorites() {
        const data = localStorage.getItem('wechat_favorites');
        return data ? JSON.parse(data) : [...this.DEFAULT_FAVORITES];
    },
    saveFavorites(favorites) { localStorage.setItem('wechat_favorites', JSON.stringify(favorites)); },

    getGlobalPrompt() {
        const raw = localStorage.getItem('wechat_global_prompt') || this.DEFAULT_GLOBAL_PROMPT;
        const normalized = replaceLegacyPromptVars(raw);
        if (normalized !== raw) localStorage.setItem('wechat_global_prompt', normalized);
        return normalized;
    },
    saveGlobalPrompt(prompt) { localStorage.setItem('wechat_global_prompt', replaceLegacyPromptVars(String(prompt || ''))); },
    ...
    getPromptModules() {
        ...
        try {
            const parsed = JSON.parse(data);
            const normalizedStored = normalizePromptModules(parsed || {});
            const merged = { ...this.DEFAULT_PROMPT_MODULES, ...normalizedStored };
            const parsedText = JSON.stringify(parsed || {});
            const normalizedText = JSON.stringify(normalizedStored);
            if (parsedText !== normalizedText) {
                try {
                    localStorage.setItem('wechat_prompt_modules', JSON.stringify(normalizedStored));
                } catch (e) {}
            }
            return merged;
        } catch (e) {
    ...
    savePromptModules(modules) { localStorage.setItem('wechat_prompt_modules', JSON.stringify(normalizePromptModules(modules))); },

    getPromptScenarioTemplates() {
        const data = localStorage.getItem('wechat_prompt_scenario_templates');
        if (!data) return { ...this.DEFAULT_PROMPT_SCENARIO_TEMPLATES };
        try {
            const parsed = JSON.parse(data) || {};
            const normalized = normalizePromptTemplateMap(parsed);
            const merged = { ...this.DEFAULT_PROMPT_SCENARIO_TEMPLATES, ...normalized };
            if (JSON.stringify(parsed) !== JSON.stringify(normalized)) {
                try { localStorage.setItem('wechat_prompt_scenario_templates', JSON.stringify(normalized)); } catch (e) {}
            }
            return merged;
        } catch (e) {
            return { ...this.DEFAULT_PROMPT_SCENARIO_TEMPLATES };
        }
    },
    savePromptScenarioTemplates(templates) { localStorage.setItem('wechat_prompt_scenario_templates', JSON.stringify(normalizePromptTemplateMap(templates))); },
    ...
    getPromptScenarioContext() {
        ...
        return {
            ...
            callRecentChatLines: clamp(merged.callRecentChatLines, 0, 100, this.DEFAULT_PROMPT_SCENARIO_CONTEXT.callRecentChatLines),
            callLiveHistoryLines: clamp(merged.callLiveHistoryLines, 1, 100, this.DEFAULT_PROMPT_SCENARIO_CONTEXT.callLiveHistoryLines),
  ...
  saveSummaryConfig(config) {
    ...
    getSummaryHistories() {
        ...
        if (!data) return {};
        try {
            return normalizeSummaryHistoryMap(JSON.parse(data));
        } catch (e) {
            return {};
        }
    },
    saveSummaryHistories(histories) { localStorage.setItem('wechat_summary_histories', JSON.stringify(normalizeSummaryHistoryMap(histories))); },
...
.btn-danger{background:#fa5151 !important;color:#fff !important;min-height:var(--wx-danger-btn-h) !important;height:var(--wx-danger-btn-h) !important;max-width:calc(var(--wx-danger-btn-max) * 1%) !important;width:100% !important;margin-left:auto !important;margin-right:auto !important;border-radius:var(--wx-danger-btn-radius) !important;padding:0 var(--wx-danger-btn-px) !important;font-size:calc(var(--wx-danger-btn-font) * var(--wx-text-scale)) !important;display:flex !important;align-items:center !important;justify-content:center !important;}
    ...
    return list
        .map((item) => {
            ...
            return {
                ...
                chatBackground: raw.chatBackground && typeof raw.chatBackground === 'object' ? { ...raw.chatBackground } : null,
...
function getSessionAvatar(session) {
    ...
    if (!isGroupSession(session)) {
        const contact = (State.contacts || []).find((c) => String(c?.id || '') === String(session.contactId || session.id || ''));
        return String(contact?.avatar || 'user');
    }
    return String(session.avatar || 'group').trim() || 'group';
}
...
async function removeFavoriteById(favoriteId) {
    ...
    if (idx < 0) return;
    const ok = await WeChatUI.showConfirm('取消收藏', '确定取消收藏这条消息吗？', '取消收藏', '返回');
    if (!ok) return;
...
function renderFavoritesPage() {
    ...
    const items = Array.isArray(State.favorites) ? State.favorites.slice().sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0)) : [];
    listEl.innerHTML = '';
    if (items.length === 0) {
        ...
        row.innerHTML = `
            <div class="favorite-item-main">
                <div class="favorite-item-head">
                    <span class="favorite-item-contact">${escapeHtml(item.contactName || '聊天')}</span>
...
function buildRecallEventPayload(contact, actorRole, summary, options = {}) {
    ...
    const aiText = isUser
        ...
        : `[聊天事件] ${(actorLabel || '对方')}撤回了一条消息：${msgSummary}`;
    return {
        type: 'event',
        event: 'recall',
...
function buildPatEventText(contact, actorRole, options = {}) {
    ...
    const userLabel = String(options.userName || getUserProfileForContact(options.target || contact).nickname || '用户').trim() || '用户';
...
function refreshUnrestrictedHeaderButton() {
    ...
    const stateEl = document.getElementById('chat-unrestricted-btn-state');
    ...
    stateEl.textContent = isActive ? String(summary.enabledCount) : (summary.normalized.enabled ? '空' : '关');
...
function getDraftPromptScenarioContextFromForm() {
    return {
        ...
        callRecentChatLines: clampInt(document.getElementById('prompt-call-recent-chat-lines')?.value, 0, 100, 12),
        callLiveHistoryLines: clampInt(document.getElementById('prompt-call-live-history-lines')?.value, 1, 100, 16),
...
function buildGroupMemberPromptProfiles(members = []) {
...
function getGroupAiVisibleContextLimit(session, member = null) {
    ...
    return Math.max(1, parseInt(member?.contextLimit ?? 150, 10) || 150);
}
...
function buildGroupReplyPromptBundle(session, member, options = {}) {
    ...
    const memberProfilesText = buildGroupMemberPromptProfiles(members);
    ...
    const vars = {
        char: String(member?.name || getContactDisplayName(member) || '群成员').trim() || '群成员',
        role: String(member?.name || getContactDisplayName(member) || '群成员').trim() || '群成员',
    ...
    systemLines.push([
        ...
        '- 但 `WX_MONEY_ACCEPT` 不算乱用 action：如果群里刚有人给你发了红包或转账，而且按你的人设会顺手领，就可以直接输出这个 action。',
        '- `WX_PAT` 和 `WX_RECALL_LAST / WX_RECALL_OFFSET` 也允许用，但要克制，只有你真会这么做时才用。',
        '- 如果要撤回，只能撤回你自己刚刚发过的消息，不能撤回别的群成员的消息。',
        '- `WX_CALL_START` 允许你发起语音/视频通话，来电人就是你自己。',
        '- `WX_AVATAR_SET` 只会修改你自己的头像，不会改别的群成员头像；`source:"last_user_image"` 会拿用户最近在本群发的图片。',
        '- `WX_REMARK_SET` 在群聊里表示修改群内昵称：target:"user" 是你给用户设置你眼里的群内昵称，target:"contact"/"self"/"member" 是用户给你设置群内昵称。',
        '- 红包和转账如果要发，请直接用 text/text_message 的三行卡片格式输出。',
        '- 回复尽量 1 到 2 句，简短自然，不要写说明，不要加 markdown。'
    ].join('\n'));
    if (!session?.allowOfflineSameLocation) {
    ...
    const forceReplyHint = options.forceReply
        ...
        : '';
    const userPrompt = `这是最近群聊记录：\n${lines || '（暂无历史消息）'}\n\n请你以「${getContactDisplayName(member)}」身份，在这个群里顺着刚才的话自然接一句。${forceReplyHint}`;
    return {
        systemPrompt,
        userPrompt,
        stickerCandidates,
...
function updatePromptPreviewContextHint(mode) {
    ...
    if (previewMode === 'group-chat') {
        ...
        hintEl.textContent = runtime.isLive
            ? `当前显示的是群聊「${getSessionDisplayName(runtime.session)}」本轮会发给 AI 的提示词预览，会读取最近 ${historyCount} 条群消息，并按最近一次用户发言生成。`
            : `当前没有可用的真实群聊上下文，下面先显示一份群聊互动示例提示词。触发词示例：${triggerText || '无'}`;
        return;
    }
...
function refreshGroupPromptModule() {
    ...
    try {
        ...
        if (hintEl) {
            const historyCount = Array.isArray(runtime.history) ? runtime.history.length : 0;
            hintEl.textContent = runtime.isLive
                ? `当前显示的是群聊「${getSessionDisplayName(runtime.session)}」实际会发给 AI 的提示词，已读取最近 ${historyCount} 条群消息，并按最近一次用户发言来生成。`
                : `当前没有可用的真实群聊上下文，这里先显示一份群聊互动示例提示词。`;
        }
    } catch (err) {
...
function bindPromptPreviewEvents() {
    ...
    [
        'global-prompt',
        'prompt-image-length',
        'prompt-image-min-chars',
        'prompt-image-max-chars',
        'prompt-extra-rules',
        'prompt-sticker-candidate-count',
        'prompt-role-template',
        'prompt-protocol-template',
        'prompt-human-template',
        'prompt-proactive-template',
        'prompt-moments-post-system-template',
        'prompt-moments-post-user-template',
        'prompt-moments-interaction-system-template',
        'prompt-moments-interaction-user-template',
        'prompt-moments-reply-system-template',
        'prompt-moments-reply-user-template',
        'prompt-moments-social-system-template',
        'prompt-moments-social-user-template',
        'prompt-call-system-template',
        'prompt-call-auto-reply-template',
        'prompt-summary-mini-template',
        'prompt-summary-mega-template',
        'prompt-moments-recent-chat-lines',
        'prompt-moments-memory-max-chars',
        'prompt-moments-reply-batch-size',
        'prompt-moments-social-max-actors',
        'prompt-moments-social-max-comments',
        'prompt-call-recent-chat-lines',
        'prompt-call-live-history-lines',
        'prompt-call-auto-user-count',
        'prompt-preview-mode'
    ].forEach((id) => {
        const el = document.getElementById(id);
        if (!el) return;
...
function createGroupSessionFromForm() {
    ...
    const session = {
        ...
        avatar: String(avatarEl?.value || '').trim() || 'group',
        memberIds: checked,
        ownerId: 'me',
        announcement: '',
...
function hydrateGroupChatMorePage(session) {
    ...
    const offlineSameLocationToggle = document.getElementById('group-chat-offline-same-location-toggle');
    ...
    if (offlineSameLocationToggle) offlineSameLocationToggle.classList.toggle('active', !!session.allowOfflineSameLocation);
    ...
    const messageCount = getSummarizableChatMessages(session.id, { includeArchived: true }).length;
    if (manualSummaryCountEl) manualSummaryCountEl.textContent = `当前共 ${messageCount} 条对话`;
    if (manualSummaryHintEl) {
        manualSummaryHintEl.textContent = messageCount > 0
            ? '按群聊消息条数编号，包含起止；会生成一条小总结，但不会删除聊天原文'
            : '当前群聊还没有可总结的消息';
    }
...
function getMentionedGroupMembers(session, userText = '') {
    ...
    return members
        .map((member) => {
            const aliases = getGroupMemberMentionAliases(member);
            let matchedIndex = -1;
...
function getOrderedGroupReplyMembers(session, userText = '') {
    const members = getGroupMemberContacts(session);
    ...
    const ordered = [
        ...mentioned,
        ...rotated.filter((member) => !mentionedIds.has(String(member?.id || '')))
    ];
...
async function runGroupAiReply(session, options = {}) {
    ...
    if (State.aiTask?.running) {
        if (String(State.aiTask.contactId || '') === sid) {
            showToast('正在生成中...');
        } else {
            showToast('已有生成任务进行中');
        }
    ...
    const orderedMembers = getOrderedGroupReplyMembers(session, triggerText);
    ...
    try {
        ...
        for (let i = 0; i < orderedMembers.length; i++) {
            ...
            const recallQueryText = buildMemoryRecallQueryText(memberHistory, triggerText);
            const recalledSummaryText = await recallSummaryChunks(session, recallQueryText, 2);
            ...
            await fetchGroupAIResponse(session, member, {
            ...
            respondedMemberIds.push(String(member?.id || ''));
        }
...
async function fetchGroupAIResponse(session, member, options = {}) {
    ...
    try {
        ...
        const cleanedReply = reply.replace(/^```json\s*/, '').replace(/```\s*$/, '');
        let output = [];
            ...
            if (Array.isArray(parsed)) {
                ...
                let parsedArr = normalizeQuoteMessagesInPayload(
                    normalizePseudoMediaMessagesInPayload(
                        normalizeStickerMessagesInPayload(parsed.slice(), member.id)
                    ),
                    session.id
                );
                ...
                for (const act of actionItems) {
                    ...
                    if (cmd === 'WX_MONEY_ACCEPT') {
                        ...
                        applyGroupAvatarSet(session, member, act);
                    }
                }
...
function getAiVisibleHistory(contactId) {
    ...
    const history = Array.isArray(State.chatHistories?.[id]) ? State.chatHistories[id] : [];
    return history.filter((item) => item && !isArchivedForAiMessage(item));
}
...
function getSummarizableChatMessages(contactId, options = {}) {
    ...
    history.forEach((item, index) => {
        ...
        result.push(withIndexes ? { ...item, _historyIndex: index } : item);
    });
    return result;
}
...
function expandAiDisplayItems(items, options = {}) {
    ...
    for (const it of (Array.isArray(items) ? items : [])) {
        ...
        if (t === 'text' || t === 'text_message') {
            ...
            if (shouldKeepStructuredTextBubble(raw)) {
                out.push({ ...it, type: 'text', content: raw });
...
function getHistoryUnitCount(history) {
    ...
    return list.reduce((sum, item) => {
        if (!item) return sum;
        if (isArchivedForAiMessage(item)) return sum;
...
function applyApiStreamUIFromConfig(config) {
    const toggle = document.getElementById('api-stream-toggle');
    if (!toggle) return;
    const enabled = config?.streamEnabled !== false;
    toggle.classList.toggle('active', enabled);
}
...
function renderChatModelDropdown() {
    ...
    const headerEl = titleEl?.closest?.('.chat-model-dropdown-header') || titleEl?.parentElement;
    if (headerEl) {
        let badge = document.getElementById('chat-model-latency-badge');
        if (!badge) {
            badge = document.createElement('span');
            badge.id = 'chat-model-latency-badge';
            badge.className = 'chat-model-latency-badge';
            headerEl.appendChild(badge);
        }
        const cidForBadge = String(State.currentContactId || '');
        const info = cidForBadge ? State.aiLastTimings?.[cidForBadge] : null;
        const ms = Number(info?.firstMs ?? NaN);
        const label = Number.isFinite(ms) ? `${(ms / 1000).toFixed(ms >= 10000 ? 0 : 1)}s` : '--';
        badge.textContent = `首条 ${label}`;
    }
    ...
    const session = getCurrentSession();
    const cid = String(State.currentContactId || '');
    const contact = cid ? State.contacts.find(c => String(c?.id || '') === cid) : null;
    const sid = String(session?.id || '');
    const isGroup = isGroupSession(session);
    const running = sid ? isAiRunningForSession(sid) : false;
    const interrupted = sid ? !!State.aiInterrupted?.[sid] : false;
    const canUseActions = !!session && (!contact || !contact.isBlocked);
    const regenMeta = canUseActions ? '<span class="chat-model-item-meta">重回</span>' : '<span class="chat-model-item-meta">不可用</span>';
    const interruptLabel = running ? '中断生成' : (interrupted ? '恢复生成' : '中断生成');
    const interruptIcon = running ? 'fa-pause-circle' : (interrupted ? 'fa-play-circle' : 'fa-pause-circle');
    const interruptMeta = running ? '<span class="chat-model-item-meta">执行</span>' : (interrupted ? '<span class="chat-model-item-meta">继续</span>' : '<span class="chat-model-item-meta">无任务</span>');
    const actionDisabledStyle = canUseActions ? '' : 'opacity:0.5;';
    const actionHtml = `
        <div class="chat-model-item" data-action="ai-regen" style="${actionDisabledStyle}">
            <span style="display:flex; align-items:center; gap:8px;"><i class="fas fa-redo"></i><span>重新生成</span></span>
            ${regenMeta}
        </div>
        <div class="chat-model-item" data-action="ai-interrupt" style="${actionDisabledStyle}">
            <span style="display:flex; align-items:center; gap:8px;"><i class="fas ${interruptIcon}"></i><span>${interruptLabel}</span></span>
            ${interruptMeta}
        </div>
        <div style="height:1px; background:rgba(0,0,0,0.06); margin:6px 4px;"></div>
    `;

    const configsHtml = configs.map((cfg) => {
    ...
    listEl.innerHTML = actionHtml + configsHtml;

    listEl.onclick = async (e) => {
        const regenEl = e.target.closest('[data-action="ai-regen"]');
        if (regenEl) {
            if (!session) {
                showToast('请先打开一个聊天');
                return;
            ...
            try {
                if (isGroup) {
                    await regenerateGroupAiReply(session, { userText: getLatestGroupUserTriggerText(session) });
                } else {
                    await regenerateAiReplyForContact(contact, { allowMoneyActions: true });
                }
            } catch (error) {
                addSystemMessage(isGroup ? '群聊重新生成失败，请检查API设置' : '重新生成失败，请检查API设置');
            } finally {
        ...
        const interruptEl = e.target.closest('[data-action="ai-interrupt"]');
        if (interruptEl) {
            if (!session) {
                showToast('请先打开一个聊天');
                return;
            ...
            if (isAiRunningForSession(sid)) {
                if (!interruptAiForSession(sid)) showToast('当前没有生成任务');
                closeChatModelDropdown();
                return;
            }
            if (State.aiInterrupted?.[sid]) {
                try {
                    if (isGroup) {
                        await resumeGroupAiReply(session, { userText: getLatestGroupUserTriggerText(session) });
                    } else {
                        await resumeAiReplyForContact(contact, { allowMoneyActions: true });
                    }
                } catch (error) {
                    addSystemMessage(isGroup ? '群聊恢复失败，请检查API设置' : '恢复失败，请检查API设置');
                } finally {
                    closeChatModelDropdown();
                }
                return;
...
function getDraftApiConfigFromForm() {
    ...
    const streamEnabled = document.getElementById('api-stream-toggle')?.classList.contains('active') || false;
    return {
...
async function testCurrentApiStreamSupport() {
    const cfg = getDraftApiConfigFromForm();
    if (cfg.useProxy) {
        if (!cfg.proxyUrl) {
            showToast('请先填写代理地址');
    ...
    try {
        ...
        if (!response.ok) {
            showToast(`测试失败：HTTP ${response.status}`);
            return;
        }
        ...
        showToast('不支持流式：返回的不是 SSE');
    } catch (e) {
        const msg = (e && e.message) ? e.message : '网络错误';
        if (/AbortError/i.test(msg) || /aborted/i.test(msg)) {
            if (timedOut) {
                showToast('测试超时：可能不支持SSE或代理/网络把流“憋住”了');
            }
            return;
        }
        if (/Failed to fetch|NetworkError/i.test(msg)) {
...
async function fetchAndPickModelsForVectorConfig() {
    const modelInput = document.getElementById('vector-model-input');
    const apiUrl = String(document.getElementById('vector-api-url-input')?.value || '').trim();
    const apiKey = String(document.getElementById('vector-api-key-input')?.value || '').trim();
    const currentApiConfig = State.apiConfigs.find(c => c.id === State.currentApiId);
    const fallbackKey = String(currentApiConfig?.apiKey || '').trim();
    const tempConfig = {
        useProxy: false,
        proxyUrl: '',
        apiUrl: normalizeEmbeddingsUrl(apiUrl),
    ...
    try {
        const models = await fetchModelsListForConfig(tempConfig);
        const uniq = Array.from(new Set(models)).sort((a, b) => String(a).localeCompare(String(b)));
        const candidates = filterLikelyEmbeddingModels(uniq);
        if (uniq.length === 0) {
            showToast('未获取到模型列表');
            return;
        }
        const cur = String(modelInput?.value || '').trim();
        const tmp = { model: cur, models: cur ? [cur] : [] };
        openModelPickerModal(tmp, candidates, {
            single: true,
            title: '选择向量模型',
            onConfirm: ({ current }) => {
                if (modelInput) modelInput.value = current || '';
                if (!State.vectorConfig) State.vectorConfig = Storage.getVectorConfig();
                State.vectorConfig.model = String(current || '').trim();
                showToast(`已选择：${current}`);
            }
        });
    } catch (e) {
...
function initDiscoverPage() {
    ...
    document.getElementById('prompt-inject-entry')?.addEventListener('click', () => {
        ...
        const callRecentChatLinesEl = document.getElementById('prompt-call-recent-chat-lines');
        const callLiveHistoryLinesEl = document.getElementById('prompt-call-live-history-lines');
        ...
        if (callRecentChatLinesEl) callRecentChatLinesEl.value = scenarioContext.callRecentChatLines ?? 12;
        if (callLiveHistoryLinesEl) callLiveHistoryLinesEl.value = scenarioContext.callLiveHistoryLines ?? 16;
        ...
        if (previewModeEl) {
            if (!previewModeEl.value || previewModeEl.value === 'group-chat') {
    ...
    document.querySelectorAll('#prompt-page .prompt-reset-btn').forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            if (category === 'chat') {
                ...
                document.getElementById('prompt-call-recent-chat-lines').value = Storage.DEFAULT_PROMPT_SCENARIO_CONTEXT.callRecentChatLines;
                document.getElementById('prompt-call-live-history-lines').value = Storage.DEFAULT_PROMPT_SCENARIO_CONTEXT.callLiveHistoryLines;
    ...
    if (imageAddBtn) {
        imageAddBtn.onclick = async () => {
            ...
            if (Array.isArray(result.value)) {
                ...
                openMomentsComposeModal._images.push(String(result.value).trim());
            }
            refreshImages();
        };
    }
...
function ensureMomentsProfileBindings() {
    ...
    if (avatarEl && !avatarEl.dataset.bound) {
        ...
        avatarEl.addEventListener('click', async (e) => {
            e.stopPropagation();
            const result = await WeChatUI.showImagePicker('朋友圈头像', '输入头像图片URL（http(s):// 或 base64）', String(State.user?.avatar || ''));
            if (!result?.value) return;
    ...
    if (coverEl && !coverEl.dataset.bound) {
        ...
        coverEl.addEventListener('click', async () => {
            const result = await WeChatUI.showImagePicker('朋友圈封面', '输入封面图片URL（http(s):// 或 base64）', String(State.user?.momentsCover || ''));
            if (!result?.value) return;
...
async function generateNpcMomentSocialForContact(contact, moment) {
    ...
    (Array.isArray(obj.actors) ? obj.actors : []).slice(0, maxActors).forEach((item, idx) => {
        ...
        actorMap.set(id, {
            type: 'npc',
            id,
            nickname: String(item?.nickname || item?.name || `NPC${idx + 1}`).trim() || `NPC${idx + 1}`,
    ...
    (Array.isArray(obj.likes) ? obj.likes : []).forEach((item) => {
        ...
        if (!actor) return;
        likes.push({ byType: 'npc', byId: actor.id, byNickname: actor.nickname, at: Date.now() - Math.floor(Math.random() * 25 * 60 * 1000) });
    });
    ...
    (Array.isArray(obj.comments) ? obj.comments : []).slice(0, maxComments).forEach((item, idx) => {
        ...
        const replyActor = actorMap.get(replyToActorId);
        comments.push({
...
function renderContactMomentsPage(contactId) {
    ...
    if (coverEl) {
        ...
        newCoverEl.addEventListener('click', async () => {
            const result = await WeChatUI.showImagePicker('更换背景', '输入背景图片URL（http(s):// 或 base64）', String(contact.momentsBackground || ''));
            if (result?.value) {
...
function bindModalEvents() {
    ...
    document.getElementById('contact-tags-entry')?.addEventListener('click', () => {
        openContactTagsPage();
    });
    document.getElementById('group-chats-entry')?.addEventListener('click', () => {
        openGroupListPage();
    });
    document.getElementById('contact-tags-back-btn')?.addEventListener('click', () => {
        hidePage('contact-tags-page');
    });
    document.getElementById('group-list-back-btn')?.addEventListener('click', () => {
        hidePage('group-list-page');
    });
    document.getElementById('group-create-open-btn')?.addEventListener('click', () => {
        openGroupCreatePage();
    });
    document.getElementById('group-create-back-btn')?.addEventListener('click', () => {
        hidePage('group-create-page');
    });
    document.getElementById('group-create-search')?.addEventListener('input', () => {
        renderGroupCreateMemberList();
    });
    document.getElementById('group-create-save-btn')?.addEventListener('click', () => {
        const session = createGroupSessionFromForm();
        if (!session) return;
        hidePage('group-create-page');
        hidePage('group-list-page');
        openSession(session);
...
function openRoleModal(contact = null) {
    ...
    const tagsInput = document.getElementById('role-tags');
    ...
    if (contact) {
        ...
        if (tagsInput) tagsInput.value = normalizeContactTags(contact.tags).join('，');
...
function saveRole() {
    ...
    const tags = normalizeContactTags(document.getElementById('role-tags')?.value || '');
...
const WeChatUI = {
    ...
    showAlert: function(title, message, confirmText = '知道了') {
        return new Promise((resolve) => {
            const modal = document.getElementById('wechat-confirm-modal');
            const overlay = document.getElementById('wechat-confirm-overlay');
            const titleEl = document.getElementById('wechat-confirm-title');
    ...
    showImagePicker: function(title, placeholder = '输入图片URL（http(s):// 或 base64）', defaultValue = '', options = {}) {
        return new Promise((resolve) => {
            let modal = document.getElementById('wechat-image-picker-modal');
            if (!modal) {
                modal = document.createElement('div');
                modal.id = 'wechat-image-picker-modal';
                modal.className = 'modal wechat-dialog-modal';
                modal.innerHTML = `
                    <div class="modal-overlay"></div>
                    <div class="wechat-dialog">
                        <div class="wechat-dialog-title" id="wechat-image-picker-title">选择图片</div>
                        <div class="wechat-dialog-body">
                            <p class="wechat-dialog-text" style="margin-bottom:10px;">可直接输入图片 URL，或者上传本地图片</p>
                            <input type="text" id="wechat-image-picker-input" class="wechat-dialog-input" placeholder="">
                            <input type="file" id="wechat-image-picker-file" accept="image/*" hidden>
                            <button class="wechat-dialog-btn cancel" id="wechat-image-picker-upload" style="width:100%;margin-top:12px;border:1px solid #e5e5ea;">上传本地图片</button>
                        </div>
                        <div class="wechat-dialog-footer">
                            <button class="wechat-dialog-btn cancel" id="wechat-image-picker-cancel">取消</button>
                            <button class="wechat-dialog-btn confirm" id="wechat-image-picker-confirm">确定</button>
                        </div>
                    </div>
                `;
            ...
            const titleEl = document.getElementById('wechat-image-picker-title');
            const inputEl = document.getElementById('wechat-image-picker-input');
            const fileEl = document.getElementById('wechat-image-picker-file');
            const uploadBtn = document.getElementById('wechat-image-picker-upload');
            const cancelBtn = document.getElementById('wechat-image-picker-cancel');
            const confirmBtn = document.getElementById('wechat-image-picker-confirm');

            if (!overlay || !titleEl || !inputEl || !fileEl || !uploadBtn || !cancelBtn || !confirmBtn) {
                resolve(null);
                return;
            }
            ...
            inputEl.placeholder = placeholder || '输入图片URL（http(s):// 或 base64）';
            inputEl.value = defaultValue || '';
            fileEl.multiple = allowMultiple;
            fileEl.value = '';
            ...
            confirmBtn.onclick = () => {
                ...
                if (!value) {
                    showToast('请输入图片URL，或点击上传本地图片');
                    inputEl.focus();
                    return;
                }
                cleanup();
                resolve({ type: 'url', value });
            };
            ...
            fileEl.onchange = () => {
                ...
                files.forEach((file, index) => {
                    const reader = new FileReader();
                    reader.onload = (ev) => {
                        results[index] = String(ev?.target?.result || '');
                        loadedCount += 1;
                        if (loadedCount === files.length && !failed) {
                            cleanup();
                            resolve({ type: 'local', value: allowMultiple ? results.filter(Boolean) : (results[0] || '') });
                        }
                    };
                    reader.onerror = () => {
                        if (failed) return;
    ...
    showCallPage: function(contact, type, onHangup, options = {}) {
        ...
        const getCallContextHistory = () => {
            const baseHistory = getAiVisibleHistory(contact.id);
            const scenarioContext = State.promptScenarioContext || Storage.DEFAULT_PROMPT_SCENARIO_CONTEXT;
            const limit = clampInt(scenarioContext.callRecentChatLines, 0, 100, Storage.DEFAULT_PROMPT_SCENARIO_CONTEXT.callRecentChatLines);
            return baseHistory
                .slice(-limit)
                .map((msg) => {
        ...
        const requestCallAI = async (userText, signal) => {
            ...
            const systemPrompt = buildCallSystemPrompt(contact, type, callSummaryMemoryText);
            ...
            const scenarioContext = State.promptScenarioContext || Storage.DEFAULT_PROMPT_SCENARIO_CONTEXT;
            const liveHistoryLimit = clampInt(scenarioContext.callLiveHistoryLines, 1, 100, Storage.DEFAULT_PROMPT_SCENARIO_CONTEXT.callLiveHistoryLines);
            const recent = callHistory.slice(-liveHistoryLimit);
            recent.forEach((m) => {
                messages.push({ role: m.role, content: String(m.content || '') });
            });
            messages.push({ role: 'user', content: String(userText || '') });
            rememberPromptTrace(contact.id, 'call', [
                `【System】\n${systemPrompt}`,
                chatContextHistory.length > 0 ? `【最近常规聊天记录】\n${chatContextHistory.map((m) => `${m.role === 'assistant' ? (contact?.name || '对方') : '我'}：${m.content}`).join('\n')}` : '',
                recent.length > 0 ? `【最近通话上下文】\n${recent.map((m) => `${m.role === 'assistant' ? (contact?.name || '对方') : '我'}：${String(m.content || '')}`).join('\n')}` : '',
                `【本轮用户输入】\n${String(userText || '')}`
            ].filter(Boolean).join('\n\n'));
        ...
        const talkToAIInCall = async (userText) => {
            ...
            try {
                ...
                for (const line of lines) {
                    ...
                    const entry = { id: `call_${++callMessageSeq}`, role: 'assistant', content: line };
                    callHistory.push(entry);
                    appendCallFeed('ai', line, { historyId: entry.id });
                }
            } catch (e) {
                const isAbort = e && (e.name === 'AbortError' || String(e.message || '').toLowerCase().includes('aborted'));
                if (isAbort) {
...
function openSession(sessionLike) {
    ...
    if (batchReplyBtn) batchReplyBtn.title = isGroupSession(session) ? '让群成员回复' : '让TA回复';
    const input = document.getElementById('message-input');
    if (input) input.placeholder = isGroupSession(session) ? '发消息，输入 @ 可选成员' : '';
    renderChatMessages();
    initChatList();
...
function hydrateChatMorePage(contact) {
    ...
    const messageCount = getSummarizableChatMessages(contact.id).length;
    if (manualSummaryCountEl) manualSummaryCountEl.textContent = `当前共 ${messageCount} 条对话`;
    if (manualSummaryHintEl) manualSummaryHintEl.textContent = messageCount > 0
        ? `按聊天条数编号，包含起止；当前可选 1 - ${messageCount}，不影响自动总结和聊天记录`
        : '当前还没有可总结的聊天内容';
    if (manualSummaryStartEl) {
        manualSummaryStartEl.max = String(Math.max(1, messageCount));
        if (messageCount <= 0) {
...
function bindChatEvents() {
    ...
    document.getElementById('group-chat-offline-same-location-toggle')?.addEventListener('click', function () {
        const session = getCurrentSession();
        if (!isGroupSession(session)) return;
        this.classList.toggle('active');
        session.allowOfflineSameLocation = this.classList.contains('active');
        Storage.saveSessions(State.sessions);
        hydrateGroupChatMorePage(session);
        refreshGroupPromptModule();
        showToast(session.allowOfflineSameLocation ? '已允许群成员线下同地出现' : '已恢复默认远程场景');
    });
    ...
    document.getElementById('group-chat-avatar-cell')?.addEventListener('click', async () => {
        const session = getCurrentSession();
        if (!isGroupSession(session)) return;
        const current = String(session.avatar || '').trim();
        const result = await WeChatUI.showImagePicker('修改群头像', '输入头像图片URL（http(s):// 或 base64），也可以留空后手动填 group / user 等标识', current);
        if (!result) return;
        const val = String(result.value || '').trim();
        session.avatar = val || 'group';
        Storage.saveSessions(State.sessions);
    ...
    const openChatBgPicker = async () => {
        const target = getChatBackgroundEditorTarget();
        if (!target) return;
        const current = target?.chatBackground?.type === 'image' ? String(target.chatBackground.value || '') : '';
        const result = await WeChatUI.showImagePicker('更换聊天背景', '输入背景图片URL（http(s):// 或 base64）', current);
        if (!result?.value) return;
        const val = String(result.value || '').trim();
        if (!val) return;
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            const msgIndex = State.chatHistories[session.id].length - 1;

            addMessageToUI(messageContent, 'right', userProfile.avatar, msgIndex, { sessionId: session.id });
            input.value = '';
    ...
    document.getElementById('plus-video-btn')?.addEventListener('click', () => {
        ...
        if (!session?.id) return;
        const contact = getCurrentPrivateContact();
        const callTarget = contact || {
            id: session.id,
            name: getSessionDisplayName(session),
            avatar: getSessionAvatar(session)
        };
        if (plusPanel) plusPanel.classList.remove('show');
        
        WeChatUI.showActionSheet(
            // 选择视频通话
            () => { WeChatUI.showCallPage(callTarget, 'video', handleCallHangup('video'), { direction: 'outgoing' }); },
            // 选择语音通话
            () => { WeChatUI.showCallPage(callTarget, 'voice', handleCallHangup('voice'), { direction: 'outgoing' }); }
        );
...
function buildWxMoneyAcceptEventPayload(kind, actorName = '') {
    const who = String(actorName || '对方').trim() || '对方';
    const isTransfer = String(kind || '') === 'transfer';
    const display = isTransfer ? `${who}接收了你的转账` : `${who}领取了你的红包`;
    const aiText = isTransfer
        ? `[聊天事件] ${who}接收了用户发出的转账`
        : `[聊天事件] ${who}领取了用户发出的红包`;
    return {
        type: 'event',
...
function markOutgoingWxMoneyAccepted(historyId, kind, options = {}) {
    ...
    const eventPayload = buildWxMoneyAcceptEventPayload(kind, actorName || '对方');
...
function startIncomingCallFromGroupMember(session, member, callType) {
    ...
    WeChatUI.showCallPage(member, callType, (resultDesc, transcript = []) => {
        ...
        Storage.saveChatHistories(State.chatHistories);
        updateSessionPreview(session.id, `${getContactDisplayName(member)}：[通话记录]`, nowTs);
        member.lastActiveAt = nowTs;
        Storage.saveContacts(State.contacts);
        initChatList();
        renderGroupSessionsList();
...
function applyGroupRemarkSet(session, member, act) {
    ...
    if (target === 'user') {
        ...
        Storage.saveContacts(State.contacts);
        pushSystemMessageToHistory(session.id, `${memberName}已将你在群里的昵称设为「${remark}」`);
        return true;
    }
    if (target === 'contact' || target === 'self' || target === 'member') {
        ...
        pushSystemMessageToHistory(session.id, `你已将${memberName}的群内昵称设为「${remark}」`);
        return true;
    }
    return false;
}
...
function applyGroupAvatarSet(session, member, act) {
    ...
    const url = rawUrl || (source === 'last_user_image' ? getLastUserImageUrlForContact(session.id) : '');
    if (!url) return false;
    ...
    pushSystemMessageToHistory(session.id, `${getContactDisplayName(member)}更换了头像`);
    return true;
}
...
function isAiRunningForSession(sessionId) {
    return !!State.aiTask?.running && String(State.aiTask?.contactId || '') === String(sessionId || '');
}
...
async function fetchAIResponse(contact, options = {}) {
    ...
    recentHistory.forEach((msg, idx) => {
        messages.push({
            role: msg.role,
            content: annotateHistoryBubbleForModel(toApiContent(msg.content, msg.role), msg.role, idx)
        });
    });
    ...
    try {
        ...
        if (options.forceReply) {
            messages.push({
                role: 'user',
                content: `（系统）本轮禁止已读不回。请你正常回复 ${getUserProfileForContact(contact).nickname || '对方'}，必须输出至少一条可见消息（text/sticker_message/quote/image/voice），不要输出 action。`
            });
        }
        ...
        try {
            ...
            if (Array.isArray(parsed)) {
                let parsedArr = normalizeQuoteMessagesInPayload(
                    normalizePseudoMediaMessagesInPayload(
                        normalizeStickerMessagesInPayload(parsed.slice(), contact.id)
                    ),
                    contact.id
                );
...
async function triggerAutoSummary(contact) {
    ...
    const visibleMessages = getSummarizableChatMessages(contact.id, { includeArchived: false, withIndexes: true });
...
async function triggerManualSummary(contact, startIndex, endIndex) {
    if (!contact?.id) return;
    const allMessages = getSummarizableChatMessages(contact.id, { includeArchived: true });
...
function maybeQueueAutoSummaryForTarget(target) {
    ...
    const visibleMessages = getSummarizableChatMessages(targetId, { includeArchived: false });
...
function buildSystemPrompt(contact, overrides = {}) {
    ...
    prompt += `【名字映射】\n`;
    prompt += `- 你自己是：${roleDisplayName}\n`;
    prompt += `- 正在和你聊天的人是：${userDisplayName}\n`;
    prompt += `- 下面规则里的“你”都指 ${roleDisplayName}，不要把“你”理解成 ${userDisplayName}。\n\n`;
    ...
    if (summaryMemory) {
        const summaryMemoryAnchor = `以下是你们以往聊天里整理出的记忆片段。阅读时请固定理解：这里的“我”始终指${roleDisplayName}，另一方统一指${userDisplayName}。`;
        prompt += `【已归档记忆】\n${summaryMemoryAnchor}\n${summaryMemory}\n\n`;
    }
    ...
    prompt += `【微信扩展动作与特殊卡片】\n` +
        ...
        `  - 拍一拍说明：当前聊天里，你拍一拍对方后缀为「${patSuffixOutgoing}」，对方拍一拍你后缀为「${patSuffixIncoming}」。\n` +
...
function buildMomentInteractionPromptPair(contact, moment) {
    const templates = State.promptScenarioTemplates || Storage.DEFAULT_PROMPT_SCENARIO_TEMPLATES;
    const vars = getScenarioPromptVars(contact, {
        moment_text: String(moment?.text || '').trim() || '（无内容）'
    });
    return {
        systemPrompt: prependUnrestrictedPrompt(renderPromptTemplate(templates.momentsInteractionSystem || Storage.DEFAULT_PROMPT_SCENARIO_TEMPLATES.momentsInteractionSystem, vars)),
        userPrompt: renderPromptTemplate(templates.momentsInteractionUser || Storage.DEFAULT_PROMPT_SCENARIO_TEMPLATES.momentsInteractionUser, vars)
    };
}
...
function buildCallSystemPrompt(contact, type, summaryMemoryText = '') {
    const templates = State.promptScenarioTemplates || Storage.DEFAULT_PROMPT_SCENARIO_TEMPLATES;
    const summaryMemory = String(summaryMemoryText || '').trim();
    const roleName = contact?.name || '联系人';
    const userProfile = getUserProfileForContact(contact);
    const summaryMemoryAnchor = summaryMemory
        ? `以下是你们以往聊天里整理出的记忆片段。阅读时请固定理解：这里的“我”始终指${roleName}，另一方统一指${userProfile.nickname || '用户'}。`
        : '';
    const vars = getScenarioPromptVars(contact, {
    ...
    return prependUnrestrictedPrompt(renderPromptTemplate(templates.callSystem || Storage.DEFAULT_PROMPT_SCENARIO_TEMPLATES.callSystem, vars));
}
    ...
    startProactiveScheduler._timer = setInterval(async () => {
        ...
        try {
            for (const contact of (State.contacts || [])) {
                ...
                if (State.aiTask?.running) continue;
            ...
            <div style="width:min(320px, 80vw); height:min(440px, 70vh); background:#eb5442; border-radius:12px; position:relative; overflow:hidden; box-shadow:0 10px 30px rgba(0,0,0,0.3);">
                ...
                <div style="position:relative; padding-top:42px; display:flex; flex-direction:column; align-items:center;">
                    <div style="width:40px; height:40px; border-radius:4px; background:#fff; overflow:hidden; margin-bottom:10px;">
                        <img src="${escapeHtml(senderAvatar || 'images/default-avatar.png')}" style="width:100%; height:100%; object-fit:cover;" onerror="this.src='images/default-avatar.png'">
                    </div>
