...
function buildFavoriteEntry(contactId, msgData, messageIndex = -1) {
    const contact = (State.contacts || []).find((item) => item && String(item.id || '') === String(contactId || ''));
    ...
    return {
        ...
        contactName: contact ? getContactDisplayName(contact) : '未知聊天',
...
function buildSummaryTextBatches(messages, options = {}) {
    ...
    list.forEach((m) => {
        ...
        const prefix = timestamp ? `[${timestamp}][${m.role}]` : `[${m.role}]`;
        const line = `${prefix}: ${toPlainTextFromStoredContent(m.content)}`.trim();
        if (!line) return;
...
function getSummaryBasePrompt(contact, kind = 'mini') {
    const config = State.summaryConfig || Storage.getSummaryConfig();
    ...
    return renderPromptTemplate(promptText, getScenarioPromptVars(contact, {
...
async function requestSummaryContent(contactId, inputText, options = {}) {
    ...
    const contact = (State.contacts || []).find((item) => String(item?.id || '') === String(contactId || '')) || null;
    const basePrompt = getSummaryBasePrompt(contact, kind);
...
async function requestSummaryForMessages(contactId, messages, options = {}) {
    const batches = buildSummaryTextBatches(messages, options);
...
function bindChatEvents() {
    ...
    document.getElementById('chat-prompt-view-cell')?.addEventListener('click', () => {
        ...
        document.querySelectorAll('#prompt-view-category-tabs .prompt-history-tab').forEach((btn) => {
            btn.classList.toggle('active', btn.dataset.historyCategory === 'chat');
        });
        renderPromptTraceHistory(contact.id, 'chat');
    });
    ...
    document.querySelectorAll('#prompt-view-category-tabs .prompt-history-tab').forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            renderPromptTraceHistory(State.currentContactId, promptHistoryCategory);
        });
    });
    ...
    document.getElementById('memory-vectorize-btn')?.addEventListener('click', () => {
        ...
        const contact = State.contacts.find(c => c.id === State.currentContactId);
        if (!contact) return;
        vectorizeSummariesForContact(contact).catch((err) => {
    ...
    document.getElementById('memory-manual-mega-btn')?.addEventListener('click', () => {
        ...
        triggerManualMegaSummary(contact, count);
    });
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            if (State.summaryConfig?.enabled) {
                ...
                const contactId = String(contact?.id || State.currentContactId || '').trim();
                if (contactId && getHistoryUnitCount(State.chatHistories[contactId]) >= threshold && !State.summaryBusy?.[contactId]) {
                    State.summaryBusy[contactId] = true;
                    ...
                    (async () => {
                        try {
                            ...
                            State.summaryBusy[contactId] = false;
...
async function triggerAutoSummary(contact) {
    ...
    try {
        const summaryText = await requestSummaryForMessages(contact.id, messagesToSummarize, { kind: 'mini' });
...
function updateManualSummaryActionUi(contactId) {
    const id = String(contactId || '').trim();
    const currentId = String(State.currentContactId || '').trim();
    const btn = document.getElementById('chat-manual-summary-btn');
    const startEl = document.getElementById('chat-manual-summary-start');
    const endEl = document.getElementById('chat-manual-summary-end');
    ...
    const busy = !!(id && currentId === id && State.summaryBusy?.[id]);

    if (btn) {
        btn.disabled = busy;
        btn.classList.toggle('is-busy', busy);
        btn.textContent = busy ? '后台处理中...' : '手动小总结这段对话';
    }
    if (startEl) startEl.disabled = busy;
    if (endEl) endEl.disabled = busy;
    if (megaBtn) {
        megaBtn.disabled = busy;
        megaBtn.classList.toggle('is-busy', busy);
        megaBtn.textContent = busy ? '后台处理中...' : '手动大总结最近这些小总结';
    }
    if (megaCountEl) megaCountEl.disabled = busy;
}
...
async function triggerManualSummary(contact, startIndex, endIndex) {
    ...
    (async () => {
        try {
            const summaryText = await requestSummaryForMessages(contact.id, selectedMessages, { kind: 'mini' });
            if (!summaryText) {
            ...
            await maybeTriggerAutoMegaSummary(contact);
            await WeChatUI.showAlert('手动小总结完成', `已在当前联系人下保存第 ${start}-${end} 条对话的小总结`);
        } catch (err) {
...
function refreshSummaryRelatedUi(contactId) {
    ...
    if (currentContact && memoryPage?.classList.contains('active')) {
        loadMemoryView(currentContact);
    }
}
...
async function maybeTriggerAutoMegaSummary(contact) {
    ...
    const megaText = await requestMegaSummaryForItems(contact.id, picked, { includePreviousMega: true });
    ...
    refreshSummaryRelatedUi(contact.id);
    showToast(`已自动生成 1 条大总结，合并了 ${picked.length} 条小总结`);
    return true;
}
...
function startProactiveScheduler() {
    ...
    startProactiveScheduler._timer = setInterval(async () => {
        ...
        for (const contact of (State.contacts || [])) {
            ...
            contact.nextAutoMessageAt = now + intervalMs;
            Storage.saveContacts(State.contacts);
...
function loadMemoryView(contact) {
    const summaryStore = ensureSummaryStore(contact.id);
    ...
    const pendingMini = getPendingMiniSummaries(contact.id);
    const vectorStore = State.summaryVectors?.[contact.id];
    ...
    [megaContainer, miniContainer].forEach((container) => {
        container.querySelectorAll('.memory-delete-btn').forEach((btn) => {
            btn.addEventListener('click', async (e) => {
                ...
                if (!itemId) return;
                if (kind === 'mega') {
                    await deleteMegaSummary(contact.id, itemId);
                    return;
                }
...
function favoriteSelectedMessages() {
    ...
    const count = selectedMessageIndexes.size;
    if (count === 0) return;
    const result = addFavoritesFromMessages(State.currentContactId, selectedMessageIndexes);
    exitMultiSelectMode();
    renderFavoritesPage();
