    ...
    showCallPage: function(contact, type, onHangup, options = {}) {
        ...
        const openFullCallPage = (autoConnect = false) => {
            ...
            if (callAiReplyBtn) {
                ...
                bindLongPressAction(callAiReplyBtn, showCallAiDropdown, showCallAiDropdown, { thresholdMs: 180, preventDefault: true });
...
function bindChatEvents() {
    ...
    bindLongPressAction(batchReplyBtn, () => {
        ...
        if (plusPanel) plusPanel.classList.remove('show');
        showAiReplyDropdown(contact, { allowMoneyActions: true }, batchReplyBtn);
    }, { thresholdMs: 180, preventDefault: true });
...
function parseWxMoneyText(text) {
    ...
    const head = raw.match(/^\[(微信红包|微信转账)\]/);
    if (!head) return null;
    const kind = head[1] === '微信红包' ? 'redpacket' : 'transfer';
    const amountMatch = raw.match(/金额:\s*¥?\s*([0-9]+(?:\.[0-9]+)?)/);
    const amount = amountMatch ? (parseFloat(amountMatch[1]) || 0) : 0;
    const remarkMatch = kind === 'redpacket'
        ? raw.match(/备注:\s*([^\n]*)/i)
        : raw.match(/说明:\s*([^\n]*)/i);
    const remark = String(remarkMatch?.[1] || '').trim();
        ...
        if (parsed && typeof parsed === 'object') {
            if (parsed.type === 'text' || parsed.type === 'text_message') {
                ...
                if (text.startsWith('[微信红包]')) {
                    ...
                    const info = parseWxMoneyText(text);
                    const amount = info ? `¥${(+info.amount || 0).toFixed(2)}` : (text.match(/金额:\s*¥?\s*([0-9]+(?:\.[0-9]+)?)/)?.[0] || '').replace(/^金额:\s*/i, '').trim();
                    const remark = (info?.remark || text.match(/说明:\s*([^\n]*)/i)?.[1] || '').trim() || '转账';
                    const st = parsed.wxMoney?.status || '';
                    let footer = '微信转账';
...
function extractAiJsonArrayPayload(text) {
    const cleaned = String(text || '').trim().replace(/^```json\s*/i, '').replace(/```\s*$/i, '').trim();
...
function normalizeWxMoneyCardTextContent(content) {
...
function coalesceWxMoneyCardMessages(items) {
    const arr = Array.isArray(items) ? items.slice() : [];
    const out = [];
    for (let i = 0; i < arr.length; i++) {
        const item = arr[i];
        if (!item || typeof item !== 'object' || (item.type !== 'text' && item.type !== 'text_message') || typeof item.content !== 'string') {
            out.push(item);
            continue;
        }
        const normalized = normalizeWxMoneyCardTextContent(item.content);
        const next = arr[i + 1];
        const isTransfer = normalized.trim().startsWith('[微信转账]');
        const isRedpacket = normalized.trim().startsWith('[微信红包]');
        if ((isTransfer || isRedpacket) && next && typeof next === 'object' && (next.type === 'text' || next.type === 'text_message') && typeof next.content === 'string') {
            const needKey = isTransfer ? '说明:' : '备注:';
            const hasKey = normalized.includes(`\n${needKey}`) || normalized.includes(needKey);
            const nextText = String(next.content || '').trim();
            if (!hasKey && nextText.startsWith(needKey)) {
                out.push({ ...item, content: `${normalized.trim()}\n${nextText}` });
                i += 1;
                continue;
            }
        }
        out.push({ ...item, content: normalized });
    }
    return out;
}
...
async function fetchAIResponse(contact, options = {}) {
    ...
    try {
        ...
        // 解析并显示
        try {
            const parsed = extractAiJsonArrayPayload(replyContent);
            if (Array.isArray(parsed)) {
                let parsedArr = normalizeStickerMessagesInPayload(parsed.slice(), contact.id);
                parsedArr = coalesceWxMoneyCardMessages(parsedArr);
