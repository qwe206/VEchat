...
async function readStreamedReplyContent(response, signal) {
            ...
            if (line.startsWith('data:')) {
                ...
                if (dataText) {
                    try {
                        ...
                        fullText += extractStreamReplyTextDelta(payload);
                    } catch (e) {
                    }
                }
            }
...
async function fetchAIResponse(contact, options = {}) {
    ...
    try {
        ...
        const response = await fetch(targetUrl, {
            ...
            body: JSON.stringify(options.__disableStream ? requestPayload : { ...requestPayload, stream: true })
        });
        ...
        if (!response.ok) {
            ...
            if (!options.__disableStream && !options.__streamRetried && shouldRetryWithoutStream(response.status, errMsg)) {
        ...
        let replyContent = '';
        if (!options.__disableStream && /text\/event-stream/i.test(contentType) && response.body) {
